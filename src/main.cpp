...
void test1(){
    ...
    while(1)
        serialPort->Write(hex_to_string("FF010172650000FEFF"));
...
int main(){
    ...
    // build_or_inferOnePicture_INT8("attn_unet_sas_fresh.onnx", "engine_int8_sas.trtmodel", 1);

    // performance_test("engine_ft32_sas.trtmodel", "valid_dataset.txt", "valid_label.txt");
    // performance_test("engine_int8_sas.trtmodel", "valid_dataset.txt", "valid_label.txt");

    
    // string path = "engine_ft32_sas.trtmodel";      // 推理模型源文件
    string path = "engine_int8_sas.trtmodel";      // 推理模型源文件
